...
class SqliteDataSet(DataSet):
    ...
    def get_columns(self, table):
        c = self._database.get_columns(table)
...
def _init_app(state: AppState):
    dataset = state.dataset
    migrator = state.dataset._migrator
    app = state.app
        ...
        if request.method == "POST":
            if name and col_type in column_mapping:
                try:
                    ...
                    flash(
                        'Error attempting to add column "%s": %s' % (name, exc),
                        "danger",
                    )
        ...
        if request.method == "POST":
            if name in column_names:
                try:
                    ...
                    flash(
                        'Error attempting to drop column "%s": %s' % (name, exc),
                        "danger",
                    )
            ...
            if errors:
                ...
                try:
                    ...
                    app.logger.exception(
                        "Error attempting to insert row into %s.", table
                    )
                else:
        ...
        if request.method == "POST":
            ...
            if errors:
                ...
                try:
                    ...
                    app.logger.exception(
                        "Error attempting to update row from %s.", table
                    )
                else:
    ...
    def export(query, export_format, table=None):
        ...
        dataset.freeze(query, export_format, file_obj=buf, **kwargs)  # type: ignore
    ...
    def table_export(table):
        ...
        if request.method == "POST":
            ...
            selected = [
                c for c in (request.form.getlist("columns") or []) if c in col_dict
            ]
            if not selected:
    ...
    def table_import(table):
        ...
        if request.method == "POST":
            ...
            if not file_obj:
                ...
                try:
                    stream = TextIOWrapper(file_obj, encoding="utf8")  # type: ignore
        ...
        return render_template(
            "table_import.html", count=count, strict=strict, table=table
        )
    ...
    def _format_create_table(sql):
        m = column_re.search(sql)
        assert m is not None
        create_table, column_list = m.groups()
        columns = [
            "  %s" % column.strip()
...
def install_auth_handler(app: Flask, password):
...
def initialize_app(
    ...
    if password:
        install_auth_handler(app, password)
    ...
    if extensions:
        for ext in extensions:
            dataset._database.load_extension(ext)  # type: ignore

    _init_app(AppState(app, dataset))
    dataset.close()
